...
$$
\begin{aligned}
&r_1 = ak_1 \text{  }mod\text{  }2^w \\
&r_2 = ak_2 \text{  }mod\text{  }2^w
\end{aligned}
$$
其中 $r_1 \neq r_2$ 。为什么？我们有 $r_1-r_2=a(k_1-k_2)\text{ } (mod \text{ }2^w)$ 。由于 $a$ 为奇数，与 $2^w$ 互质，和 $(k_1-k_2)$ 与 $2^w$ 的最大公约数为 $2^k$ 其中整数 $k$ 满足 $0\leq k < w$，所以 $a(k_1-k_2) \text{ }mod\text{ }2^w\neq0$ 。所以  $r_1 \neq r_2$ 。此外，我们可以根据 $(r_1, r_2)$ 可以解出(见31章数论算法)，
$$
\begin{aligned}
...
\end{aligned}
$$
其中 $d = gcd(k_1-k_2,2^w)$, $x'$ 为 $d=(k_1-k_2)x + (2^w)y$ 的整数解(使用Extended-Euclid算法求出)， $i=0,1,...,d-1$ , 在d个解的奇数解。
