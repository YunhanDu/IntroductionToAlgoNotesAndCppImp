...
第一部分包含11.1节到11.3节，包含哈希表的链接法实现。

第二部分包含11.4节到11.5节，包含哈希表的开放寻址法的实现。
...
一个好的哈希函数近似满足**独立均匀哈希**：每个关键字都被等可能地哈希到m个slot中的任何一个，并与其他关键字哈希到哪个slot无关。遗憾的是，一般无法检查这一条件是否成立除非你本来就知道输入数据的关键字的概率分布。

一个好的静态哈希法产生哈希值的方式要和数据的概率分布独立。举个例子，比如除法哈希计算哈希值时除数得是一个特定的质数（11.3.1节），如果该质数和输入数据关键字的分布模式无关，该方法可能会有好结果。
