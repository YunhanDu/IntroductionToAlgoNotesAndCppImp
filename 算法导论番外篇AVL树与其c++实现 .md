...
Left-Left是case1的对称情形，对应代码为上面代码块中第22行，无需再讨论。
...
Left-Right是case2的对称情形，对应代码为上面代码块中第24~25行，无需再讨论。
...
和插入一个节点在流程上有一些相似之处的：我们还是先像二叉搜索树的删除操作一样，此时和12章二叉树的删除操作一模一样，该操作的c++实现如下(第2~19行)：
...
先正常的删除某个节点z，但是删除完该节点后，Avl树原先的高度平衡性质可能会被破坏，我们需要重平衡的操作来恢复该二叉搜索树中所有节点的平衡。

删除的重平衡

如上描述，在原二叉搜索树完成的基本插入过程后，从target节点（第一个高度可能发生变化的节点）沿着Parent指针往上找, 经过 $O(lgn)$ 到第一个高度失衡节点g，作为失衡节点的g，在不包含z的一侧

PS: 当完全按照二叉搜索树的方式删除了一个节点后
